document.addEventListener('DOMContentLoaded', function() {
    ...
    let notes = JSON.parse(localStorage.getItem('assignments')) || [];
    ...
    function renderNotes(page) {
        ...
        document.querySelectorAll('.note-title').forEach(title => {
            title.addEventListener('click', function() {
                const index = this.getAttribute('data-index');
                localStorage.setItem('selectedAssignment', JSON.stringify(notes[index]));
                window.location.href = 'Gwage-content-page.html';
            });

            title.addEventListener('click', function() {
                document.querySelectorAll('.note-title').forEach(title => title.classList.remove('clicked'));
